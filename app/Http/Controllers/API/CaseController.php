...
public function storequestion(Request $request)
{
    ...
    // $table = $workItem['work'] . '_data';
    // $assignWorkId = $data['id'];
    // $questionnaire = $workItem['questionnaire'];

    // $insertData = ['assign_work_id' => $assignWorkId];
    ...
    if (is_array($questionnaire) && array_keys($questionnaire) === range(0, count($questionnaire) - 1)) 
    {
        foreach ($questionnaire as $field) 
        {
            if (isset($field['name']) && array_key_exists('data', $field)) 
